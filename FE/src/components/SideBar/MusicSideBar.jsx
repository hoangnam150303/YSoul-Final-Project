...
export const MusicSideBar = ({ onToggle, isOpen }) => {
  ...
  const Menus = [
    ...
    { title: "Search", icon: <SearchOutlined />, path: `/searchPageMuscic` },
    { title: "Favourite", icon: <HeartOutlined />, path: `/favourite` },
    // Giữ nguyên icon class bi-film như code cũ của bạn
    { title: "Music", icon: <i className="bi bi-film text-lg"></i>, path: `/` },
  ...
  const handleIncreaseFavouriteCount = (type) => {
    try {
      dashBoardsApi.increaseFavouriteCount(type);
    } catch (error) {
      message.error("Error increasing favourite count: " + error.message);
    }
  };

  // ✨ Component con render Menu Item (Dùng chung cho Desktop & Mobile)
      ...
      >
        ...
        <div className="flex flex-col items-center pt-8 pb-4">
          <Link to={"/"} className="relative group">
            {/* 🟢 LOGO: Không dùng Gradient nền, giữ nguyên đen/trong suốt */}
            <div className="p-1 rounded-full bg-transparent transition-all duration-300 hover:scale-105">
              <img
                src="https://res.cloudinary.com/dnv7bjvth/image/upload/v1736842897/fancyai_1736839648739_gfhqk9.png"
                alt="User"
                className={`rounded-full border border-[#2a2a2a] object-cover transition-all duration-300 ${
          ...
          >
            ...
            <p className="text-gray-500 text-xs m-0">Premium User</p>
          </div>
        </div>
        ...
        >
          <div className="flex flex-col h-full bg-[#0f0f0f]">
            ...
            <div className="p-6 flex items-center gap-4 border-b border-[#2a2a2a] bg-[#141414]">
              ...
              <div>
                <h4 className="text-white font-bold m-0 text-sm">Hello!</h4>
