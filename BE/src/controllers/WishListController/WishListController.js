...
exports.addToWishList = async (req, res) => {
    try {
        ...
        const response = await WishListService.addToWishListService(type,id,user_id); // Calling the service to add the item to the wishlist
        if (!response.success) { // If the response is false, it means the item was not added successfully
            return res.status(400).json({  
                status: false,
...
exports.deleteItemFromWishList = async (req, res) => {
    try {
        ...
        const response = await WishListService.deleteItemFromWishListService(type,id,user_id); // Calling the service to delete the item from the wishlist
        if (!response.success) { // If the response is false, it means the item was not deleted successfully
            return res.status(400).json({
                status: false,
...
exports.checkIsFavorite = async (req, res) => {
    try {
        ...
        const response = await WishListService.checkIsFavoriteService(type,id,user_id); // Calling the service to check if the item is a favorite
        if (!response.success) { // If the response is false, it means the item was not found in the wishlist
            return res.status(400).json({
                status: false,
