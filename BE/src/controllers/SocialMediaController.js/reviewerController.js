...
exports.followUser = async (req, res) => {
  try {
    ...
    const response = await reviewerService.followUserService(
      userId,
      userFollowId
    ); // call followUserService from userService
    if (!response.success) {
...
exports.getAllReviewer = async (req, res) => {
  try {
    ...
    const response = await reviewerService.getAllReviewersService(
      userId,
      filter,
      search,
      pageSize,
      currentPage
    ); // call getAllFollowersService from userService
    if (!response.success) {
