...
exports.createPost = async (req, res) => {
  try {    
    ...
    if (!content || !user_id) {
      // check if content and user_id is empty
    ...
    const response = await postService.createPostService(
      ...
      image,
      user_id
    ); // call createPostService from postService
    if (!response.success) {
...
exports.updatePost = async (req, res) => {
  try {
    ...
    const response = await postService.updatePostService(
      ...
      single_id,
      image
    ); // call updatePostService from postService
    if (!response.success) {
      return res.status(400).json({ message: response.message }); // return error message
    }
...
exports.getAllPost = async (req, res) => {
  try {
    ...
    const response = await postService.getAllPostService(search, page, limit);
