...
exports.createPostService = async (
  content,
  film_id,
  single_id,
  image,
  ...
  try {
    ...
    let validSingle;
    if (single_id) {
      validSingle = await conectPostgresDb.query(
  ...
  try {
    ...
    if (image !== updatePost.image) {
      ...
      if (result) {
        ...
        // check if image is not removed
        return { success: false, message: "Image not updated" };
      }
    } // check if image is not empty
    if (film_id !== updatePost.film_id) {
      // check if film_id is not empty
      const validFilm = await Film.findById(film_id);
      if (!validFilm) {
        // check if film_id is not valid
    ...
    if (single_id !== updatePost.single_id) {
      // check if single_id is not empty

      const validSingle = await conectPostgresDb.query(
        "SELECT * FROM singles WHERE id = $1",
        [single_id]
      ); // check if single_id is valid
      if (!validSingle) {
        // check if single_id is not valid
...
exports.getAllPostService = async (search, currentPage, pageSize) => {
  try {
    const skip = (parseInt(currentPage) - 1) * parseInt(pageSize);
    const limit = parseInt(pageSize);

    // Tìm tất cả bài post kèm điều kiện search nếu có
    let postQuery = { isActive: true };
    if (search && search.trim() !== "") {
      // Lấy user IDs khớp search
      const userResult = await conectPostgresDb.query(
        `SELECT id FROM users WHERE name ILIKE $1 AND status = true`,
        [`%${search}%`]
      );
    ...
    // Lấy thông tin người dùng từ bảng Postgres
    const userIdsSet = [...new Set(posts.map((p) => p.user_id))];
    const userQuery = `SELECT * FROM users WHERE id = ANY($1) AND status = true`;
    const userResult = await conectPostgresDb.query(userQuery, [userIdsSet]);

    const userMap = {};
    userResult.rows.forEach((user) => {
      const sanitized = { ...user };
      delete sanitized.password;
      delete sanitized.vip;
...
exports.getPostByIdService = async (id) => {
  try {
    ...
    // find user by id
    const author = await conectPostgresDb.query(
      "SELECT * FROM users WHERE id = $1",
      [validPost.user_id]
    );
    ...
    const sanitizedUsers = author.rows.map((user) => {
      // delete fields not need of user
...
exports.likePostService = async (id, userId) => {
  try {
    ...
    if (!validPost || validUser.rows.length === 0) {
      // check if post and user are not valid
      return { success: false, message: "Post or user not found" }; // return error message
    }

    if (validPost.likes.length !== 0) {
      const index = validPost.likes.findIndex(
    ...
    if (validPost.user_id !== userId.toString()) {
      // check if user is not post owner
...
// this function is get post by user_id
exports.getPostByUserService = async (userId) => {
  try {
    const validUser = await conectPostgresDb.query(
      "SELECT * FROM users WHERE id = $1",
      [userId]
    ...
    const validPost = await Post.find({ user_id: userId }); // find post by user_id
    if (!validPost) {
      // check if post is not valid
      return { success: false, message: "Post not found" }; // return error message
    }
    const sanitizedUsers = validUser.rows.map((user) => {
      delete user.password;
      delete user.vip;
      delete user.status;
      delete user.authprovider;
    ...
    const result = { user: sanitizedUsers, post: validPost }; // create result
