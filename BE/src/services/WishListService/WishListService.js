...
exports.addToWishListService = async (type, id, user_id) => {
  try {
    let wishList = await WishList.findOne({ user_id: user_id }); // Check if the user already has a wishlist
    if (!wishList) {
    ...
    if (type === "film") {
      ...
      wishList.single.push({
        // Add the single item to the wishlist
...
exports.deleteItemFromWishListService = async (type, id, user_id) => {
  try {
    let wishList = await WishList.findOne({ user_id: user_id }); // Find the wishlist by user ID
    if (!wishList) {
      // If the wishlist does not exist, return false
      return { success: false };
    }
    if (type === "film") {
      // Check if the type is "film"
      wishList.film_id = wishList.film_id.filter(
        (item) => item.toString() !== id
      ); // Remove the film ID from the wishlist
    } else if (type === "single") {
      // Check if the type is "single"
      wishList.single = wishList.single.filter((item) => item.single_id !== id); // Remove the single item from the wishlist
    } else if (type === "nft") {
      // Check if the type is "nft"
      wishList.NFT_id = wishList.NFT_id.filter(
        (item) => item.toString() !== id
      ); // Remove the NFT ID from the wishlist
    } else {
      // If the type is not recognized, return false
      return { success: false };
    }
...
exports.checkIsFavoriteService = async (type, id, user_id) => {
  try {
    ...
    if (type === "film") {
      ...
      // Check if the type is "single"
      const isFavorite = wishList.single.some(
        (item) => item.single_id.toString() === id.toString()
      ); // Check if the single item is in the wishlist
      return { success: true, isFavorite }; // Return success and the favorite status
    } else if (type === "nft") {
      // Check if the type is "nft"
